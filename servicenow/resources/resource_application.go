...
func readResourceApplication(data *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := snowClient.GetObject(client.ApplicationEndpoint, data.Id(), application); err != nil {
		data.SetId("")
		return err
	}
...
func createResourceApplication(data *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := snowClient.CreateObject(client.ApplicationEndpoint, application); err != nil {
		return err
	}
...
func updateResourceApplication(data *schema.ResourceData, serviceNowClient interface{}) error {
	snowClient := serviceNowClient.(client.ServiceNowClient)
	if err := snowClient.UpdateObject(client.ApplicationEndpoint, resourceToApplication(data)); err != nil {
		return err
	}
...
func deleteResourceApplication(data *schema.ResourceData, serviceNowClient interface{}) error {
	snowClient := serviceNowClient.(client.ServiceNowClient)
	return snowClient.DeleteObject(client.ApplicationEndpoint, data.Id())
}
